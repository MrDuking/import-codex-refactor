...
// Inject defaults so the UI can show the actual resolved values.
const dbPathDisplay = DEFAULT_DB_PATH.replace(/\\/g, '\\\\').replace(/"/g, '&quot;');
const dbBackend = /\.sqlite$/i.test(DEFAULT_DB_PATH) ? 'sqlite' : 'json';
const cliProxyAuthDirDisplay = DEFAULT_CLIPROXY_AUTH_DIR.replace(/\\/g, '\\\\').replace(/"/g, '&quot;');
...
guiHtml = guiHtml.replace(
  'placeholder="C:\\\\Users\\\\you\\\\.cli-proxy-api"',
  `placeholder="${cliProxyAuthDirDisplay}" data-default="${cliProxyAuthDirDisplay}"`
);
    ...
    try {
      ...
      try {
        ...
        const result = await bulkImport({
          ...
          provider: typeof body.provider === 'string' && body.provider ? body.provider : DEFAULT_PROVIDER,
          authDir: typeof body.authDir === 'string' && body.authDir ? body.authDir : DEFAULT_CLIPROXY_AUTH_DIR,
