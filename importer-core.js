...
const FALLBACK_CLIPROXY_AUTH_DIR = path.join(os.homedir(), '.cli-proxy-api');
const LOCAL_CLIPROXY_CONFIG_CANDIDATES = [
  path.join(os.homedir(), 'AppData', 'Local', 'Programs', 'CLIProxyAPI', 'config.yaml'),
  path.join(os.homedir(), '.cli-proxy-api', 'config.yaml'),
  path.join(os.homedir(), '.config', 'cli-proxy-api', 'config.yaml'),
];

function readCliProxyAuthDirFromConfig(configPath) {
  try {
    const text = fs.readFileSync(configPath, 'utf8');
    const m = text.match(/^auth-dir:\s*["']?([^"'\r\n]+)["']?\s*$/m);
...
function detectCliProxyAuthDir() {
  for (const p of LOCAL_CLIPROXY_CONFIG_CANDIDATES) {
    if (!fs.existsSync(p)) continue;
    const fromConfig = readCliProxyAuthDirFromConfig(p);
    if (fromConfig) return path.resolve(expandUserPath(fromConfig));
  }
  return path.resolve(expandUserPath(FALLBACK_CLIPROXY_AUTH_DIR));
}

const DEFAULT_CLIPROXY_AUTH_DIR = detectCliProxyAuthDir();

function normalizeProviderName(provider) {
  const p = String(provider || '').trim().toLowerCase();
  if (p === 'cliproxy' || p === 'cli-proxy' || p === 'cli-proxy-api') return 'cliproxyapi';
  if (p === 'cliproxyapi') return 'cliproxyapi';
  return '9router';
}

function resolveCliProxyAuthDir(authDir) {
  const raw = typeof authDir === 'string' && authDir.trim() ? authDir.trim() : DEFAULT_CLIPROXY_AUTH_DIR;
  return path.resolve(expandUserPath(raw));
}
...
function normalizeEmailForFilename(email) {
  return String(email || '').trim().toLowerCase();
}

function buildCliProxyFileName({ email, planType }) {
  const safeEmail = normalizeEmailForFilename(email);
  const safePlan = normalizePlanTypeForFilename(planType);
  if (!safeEmail) throw new Error('Thiếu email để đặt tên file CLIProxyAPI');
  return safePlan ? `codex-${safeEmail}-${safePlan}.json` : `codex-${safeEmail}.json`;
}

function toCliProxyAuthFile(entry, source = {}) {
  const email =
    normalizeEmailForFilename(source.email || entry.email) ||
    normalizeEmailForFilename(entry.name);
  const accountId =
    source.chatgptAccountId ||
    (entry.providerSpecificData && entry.providerSpecificData.chatgptAccountId) ||
    null;
  const planType =
    source.chatgptPlanType ||
    (entry.providerSpecificData && entry.providerSpecificData.chatgptPlanType) ||
    '';

  if (!entry || !entry.accessToken) throw new Error('Thiếu access_token');
  if (!entry.refreshToken) throw new Error('Thiếu refresh_token');
  if (!email) throw new Error('Thiếu email');
  if (!accountId) throw new Error('Thiếu account_id');
  ...
  const auth = {
    ...
    disabled: false,
    email,
    expired: source.expiresAt || entry.expiresAt || new Date(Date.now() + 10 * 24 * 3600 * 1000).toISOString(),
    id_token: typeof source.idToken === 'string' ? source.idToken : '',
    last_refresh: source.lastRefresh || new Date().toISOString(),
    refresh_token: entry.refreshToken,
    type: 'codex',
  };
  if (!auth.id_token) throw new Error('Thiếu id_token');

  return {
    fileName: buildCliProxyFileName({ email, planType }),
    auth,
    email,
    planType: normalizePlanTypeForFilename(planType) || '',
  };
}
...
function writeCliProxyAuthFiles(rows, { authDir, log = () => {} } = {}) {
  const resolvedAuthDir = resolveCliProxyAuthDir(authDir);
  if (!fs.existsSync(resolvedAuthDir)) fs.mkdirSync(resolvedAuthDir, { recursive: true });

  const actions = [];
  const seenTargets = new Set();
  for (const row of rows) {
    if (!row || !row.entry) continue;
    const built = toCliProxyAuthFile(row.entry, row.source || {});
    const target = path.join(resolvedAuthDir, built.fileName);
    ...
    if (seenTargets.has(key)) {
      actions.push({
        ...
        name: built.fileName,
        source: row.source,
        error: 'Trùng file đích trong cùng batch import',
      });
      continue;
    }
    seenTargets.add(key);
    ...
    if (fs.existsSync(target)) {
      try {
        ...
        const newText = JSON.stringify(built.auth, null, 2);
        if (oldText === newText) action = 'unchanged';
        else {
    ...
    if (action !== 'unchanged') writeJsonAtomic(target, built.auth);
    log(`CLIProxyAPI → ${built.fileName} (${action})`);
    actions.push({
      file: row.file,
      name: built.fileName,
      source: row.source,
  ...
  const replaced = actions.filter((a) => a.action === 'replaced').length;
  const unchanged = actions.filter((a) => a.action === 'unchanged').length;
  ...
  return {
    ...
    unchanged,
    errors,
    addedEmails: actions.filter((a) => a.action === 'created').map((a) => a.source && a.source.email).filter(Boolean),
    refreshedEmails: actions.filter((a) => a.action === 'replaced').map((a) => a.source && a.source.email).filter(Boolean),
    skippedEmails: actions.filter((a) => a.action === 'unchanged').map((a) => a.source && a.source.email).filter(Boolean),
  };
}
...
async function bulkImport(opts = {}) {
  ...
  const provider = normalizeProviderName(opts.provider || DEFAULT_PROVIDER);
  const dbPath = opts.dbPath || DEFAULT_DB_PATH;
  const authDir = resolveCliProxyAuthDir(opts.authDir);
  ...
  if (provider === 'cliproxyapi') {
    const validRows = parsed.filter((p) => p.entry && !p.error);
    const preview = [];
    for (const row of validRows) {
      try {
        ...
        preview.push({
          ...
          name: built.fileName,
          source: row.source,
          target: path.join(authDir, built.fileName),
        });
      } catch (e) {
        parsed.push({ file: row.file, error: e.message });
      }
    }
    ...
    if (dryRun) {
      return {
        ok: true,
        code: parsed.some((p) => p.error) ? 3 : 0,
        dryRun: true,
        provider,
        authDir,
        wasRunning: false,
    ...
    try {
      const written = writeCliProxyAuthFiles(validRows, { authDir, log });
      const parseFails = parsed.filter((p) => p.error).length + written.errors.length;
      return {
        ok: true,
        code: parseFails > 0 ? 3 : 0,
        provider,
        authDir: written.authDir,
        wasRunning: false,
        parsed,
        actions: written.actions,
        added: written.created,
        skipped: written.unchanged,
        skippedOnly: written.unchanged,
        refreshed: written.replaced,
        addedEmails: written.addedEmails,
        skippedEmails: written.skippedEmails,
        refreshedEmails: written.refreshedEmails,
        backup: null,
        restarted: false,
      };
    } catch (e) {
      return {
        ...
        provider,
        authDir,
        message: `Lỗi ghi auth-dir CLIProxyAPI: ${e.message}`,
        parsed,
      };
    }
  }
...
module.exports = {
  ...
  _internals: {
    ...
    buildCliProxyFileName,
    toCliProxyAuthFile,
    writeCliProxyAuthFiles,
  },
};
