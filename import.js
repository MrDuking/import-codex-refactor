 ...
 *   node import.js --provider cliproxyapi         # ghi auth JSON vào CLIProxyAPI auth-dir
 *   node import.js --auth-dir C:\path\auths       # chỉ định auth-dir cho CLIProxyAPI
 *
...
function parseArgs(argv) {
  const opts = {
    ...
    authDir: DEFAULT_CLIPROXY_AUTH_DIR,
    help: false,
  };
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === '--auth-dir') opts.authDir = argv[++i];
...
(async () => {
  ...
  log(`${c.dim('Provider:')} ${opts.provider}`);
  if (String(opts.provider).toLowerCase() === 'cliproxyapi') {
    log(`${c.dim('CLIProxyAPI auth-dir:')} ${opts.authDir}`);
  } else {
  ...
  if (opts.dry) {
    log('');
    if (String(opts.provider).toLowerCase() === 'cliproxyapi') {
      log(c.yellow('[--list] Dry-run, không ghi auth-dir CLIProxyAPI.'));
      log(`${c.dim('Auth-dir đích:')} ${opts.authDir}`);
    } else {
  ...
  log('');
  if (String(result.provider || opts.provider).toLowerCase() === 'cliproxyapi') {
    log(
      c.bold('Hoàn tất:') + ' ' +
        c.green(`+${result.added} tạo mới`) + ', ' +
        c.cyan(`↻${result.refreshed || 0} thay thế`) + ', ' +
        c.yellow(`${result.skippedOnly || 0} không đổi`) +
        `\n${c.dim('Auth-dir:')} ${result.authDir || opts.authDir}`
    );
  } else {
    log(
      c.bold('Hoàn tất:') + ' ' +
